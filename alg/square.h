// Defines the particle system and composing particles for the Square Formation
...
//
// Run with square(#particles, hole probability) on the simulator command line.

#ifndef AMOEBOTSIM_ALG_Square_H
#define AMOEBOTSIM_ALG_Square_H

#include "alg/amoebotparticle.h"
#include "alg/amoebotsystem.h"

class SquareParticle : public AmoebotParticle {
 public:
  enum class State {
    Seed,
    Idle,
  ...
  SquareParticle(const Node head, const int globalTailDir,
  ...
  SquareParticle& nbrAtLabel(int label) const;
 ...
 private:
  friend class SquareSystem;
};

class SquareSystem : public AmoebotSystem  {
 public:
  // Constructs a system of SquareParticles with an optionally specified size
  ...
  // expanded.
  SquareSystem(int numParticles = 200, float holeProb = 0.2);

  // Checks whether or not the system's run of the Square formation algorithm
  // has terminated (all particles in state Finish).
  bool hasTerminated() const override;
};

#endif  // AMOEBOTSIM_ALG_Square_H
