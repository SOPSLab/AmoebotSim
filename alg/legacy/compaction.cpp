...
std::shared_ptr<LegacySystem> Compaction::instance(const unsigned int size)
{
    ...
    system->insertParticle(LegacyParticle(std::make_shared<Compaction>(State::Seed), randDir(), seed, -1));
    while(!seedComponent.empty()) {
        auto node = *seedComponent.begin();
        seedComponent.erase(seedComponent.begin());
        system->insertParticle(LegacyParticle(std::make_shared<Compaction>(State::Idle), randDir(), node, -1));
    }
