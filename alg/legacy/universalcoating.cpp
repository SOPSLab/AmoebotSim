...
namespace UniversalCoating
{
    ...
    Movement UniversalCoating::execute()
    {
        ...
        //&&qDebug()<<"id: "<<id;
        //&&qDebug()<<"expanded? "<<isExpanded()<<" phase: "<<(int)phase;
        ...
        /*if(isExpanded())
         ...
         {
         //&&qDebug()<<"i: "<<i<<outFlags[i].followIndicator<<" c: "<<outFlags[i].contrFollowIndicator<<" e: "<<outFlags[i].expFollowIndicator<<" id: "<<inFlags[i]->id;
         }
         else
         {
         //&&qDebug()<<"i: "<<i<<outFlags[i].followIndicator<<" c: "<<outFlags[i].contrFollowIndicator<<" e: "<<outFlags[i].expFollowIndicator;
         }
         }
        ...
        if(phase == Phase::LayerRoot || phase==Phase::retiredLeader || hasNeighborInPhase(Phase::LayerRoot) || hasNeighborInPhase(Phase::retiredLeader) || phase == Phase::LayerFollow)
        {
            //&&qDebug()<<"phase: "<<(int)phase<<" layer#: "<<Lnumber<<" retured neighbors: "<<NumFinishedNeighbors;
            /*if(phase == Phase::retiredLeader && hasNeighborInPhase(Phase::Static))
             ...
             //&&qDebug()<<"retired on static: old: "<<NumFinishedNeighbors<<" new: "<<newNumFinishedNeighbors<<" "<<Lnumber;
    ...
    int UniversalCoating::receiveTokenFromFollow(TokenType type)
    {
        /*  Q_ASSERT(peekAtToken(type, dir) != -1);
         outFlags[dir].complaintToken.receivedToken = true;
         return inFlags[dir]->complaintToken;*/
        currentLocData.tokens[followOutIndex][(int)type].receivedToken = true;
        return followLocData.tokens[followTokensIndex][(int)type].value;
    }
    ...
    bool UniversalCoating::allLocationFollowersExpanded()
    {
        ...
        //&&qDebug()<<"checking for all expanded- follower off surface? "<<followerOffSurface;
        //if a follower is off the surface we want to take care of them, so check if all followers off surface are expanded by returning true
        if(followerOffSurface)
        {
            for(auto it = tailLabels().cbegin(); it != tailLabels().cend(); ++it) {
                ...
                if(inFlags[label]!=nullptr && !neighborIsInPhase(label,Phase::Static) && !inFlags[label]->onSurface )//&& inFlags[label]->isContracted())//someone there
                {
                    ...
                    //if someone behind has a loc parent that is the same as the index (+in this block offsurface) pointed at me
                    if(followLocData.parentIndex == followersIndex)
                    {
