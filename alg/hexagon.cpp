...
Movement Hexagon::execute(std::array<const Flag*, 10>& flags)
{
    ...
    if(isExpanded()){
        ...
        if (state == State::Idle){
            ...
            else if (hasNeighborInState(State::Set)){
        ...
        else if (state == State::Set && !hasNeighborInState(State::Idle)){ 
            headMarkDir = -1;
            ...
            else if (!hasNeighborInState(State::Leader) && !hasNeighborInState(State::Follower)){
                auto moveDir = getMoveDir();
                setContractDir(moveDir);
                headMarkDir = moveDir;
                return Movement(MovementType::Expand, moveDir);
...
void Hexagon::setState(State _state)
{
    ...
    if (state == State::Seed){
    	...
    	headMarkColor = 0xffff00; tailMarkColor = 0xffff00; // Yellow
    }
...
int Hexagon::getMoveDir()
{
    ...
    if (state == State::Set) {
        ...
        int objectDir = firstNeighborInState(State::Set);
        if (neighborInState((objectDir+2)%6, State::Set)){
            objectDir = (objectDir+2)%6;
        }
        objectDir = (objectDir+5)%6;
        if (neighborInState(objectDir, State::Set)){
            objectDir = (objectDir+5)%6;
        }
        return labelToDir(objectDir);
    }
